# PCoA from a Bray-Curtis matrix. Reads data.csv only. Run from this directory.
...
pc <- stats::cmdscale(bray(mat), k = 2, eig = TRUE)
plot_df <- data.frame(x = pc$points[, 1], y = pc$points[, 2], group = group)
