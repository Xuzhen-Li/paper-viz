...
description: >-
  House fork of cathrynlavery/diagram-design (MIT). Draw branded architecture,
  flowcharts, sequence, state, swimlane, timeline, layers, loops, process, and
  data-flow as self-contained HTML/SVG. Trigger on 示意图 / 架构图 / 流程图 / 时序图 /
  schematic / architecture / flowchart / sequence. Keep URL/skill/folder/paste
  brand scrape. Bar, line, scatter, Gantt → nature-figure. Redraw .drawio or
  Mermaid. After picking a type, query.py routes --need <type> --house.
license: MIT
metadata:
  version: "2.4-house"
...
# Schematic Design (house fork)

Fork of [cathrynlavery/diagram-design](https://github.com/cathrynlavery/diagram-design) (MIT). Upstream text and assets stay; this file only adds house routing, Chinese triggers, and a no-brand token fallback.
...
**House extras (read before drawing):**

1. After you pick a visual type, from AI_lib root run `python3 visualization/skill/query.py routes --need <type> --house`. If a `kept` card exists, start from that HTML (under `visualization/library/entries/` or the `assets/` example it points at).
2. Brand scrape is unchanged (§0, `references/onboarding.md`). If there is **no** brand and the user gives **no** site, use [`references/house-tokens.md`](references/house-tokens.md) instead of shipping atomic-tangerine by default.
3. **Bar / line / scatter / Gantt** in the type table are quantitative. Do **not** draw them here — Read `~/.cursor/skills/nature-figure/SKILL.md`. Assets may remain for study only.
...
Then branch per the matching section of [`references/onboarding.md`](references/onboarding.md); for **(f)** follow [`references/profiles.md`](references/profiles.md). If they pick **(e)** and there is still no brand, apply [`references/house-tokens.md`](references/house-tokens.md) (promenade neutrals), not the upstream atomic-tangerine default.
...
Use for schematic visual types (§3) when a reader will learn more from a visual than from prose, a table, or a bulleted list.

**Don't use for:**

- Manuscript **data** plots (volcano, heatmap, PCA, bar/line/scatter of measurements) → **nature-figure**.
...
| Quantitative comparison across categories | **Bar chart → nature-figure** | do not draw here |
| Continuous trends over time | **Line chart → nature-figure** | do not draw here |
| Tasks and phases on a timeline | **Gantt → nature-figure** (timeline type stays here) | do not draw here |
| Distribution and correlation between two variables | **Scatter plot → nature-figure** | do not draw here |
